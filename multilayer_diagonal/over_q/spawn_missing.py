...
if __name__ == '__main__':
    ...
    # S2 = -2
    ###########################################
    node_overlap_list = [1.0]  # np.linspace(0, 1, 30)
    ###########################################

    for T1 in np.linspace(-1, -3, 21)[1:]:
        T1 = round(T1, 2)
        T2 = round(-2.0 - T1, 2)
        S1 = round(-T1 - 3.0, 2)
        S2 = round(-T2 - 3.0, 2)

        results_dir = f"res/long_res_{rules[update_str_type]}_k{k}_gap{round(T2-T1, 2)}"
        ...
        for i, node_overlap in enumerate(node_overlap_list):
            ...
            if os.path.isfile(f_name):
                ...
                command = f'run -t 100:05 -o {out_file} -e {er_file} {py_path} {script} {node_overlap} {update_str_type} {results_dir} {T1} {S1} {T2} {S2} {k}'
                os.system(command)
