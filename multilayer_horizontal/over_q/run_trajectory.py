...
if __name__ == '__main__':
    update_rule = const.BEST_RESPONSE
    k = 999

    for shared_nodes_ratio in [0.5]:
        layers_config = [
            {"b": None, "R": 1, "P": 0, "S": -1, "T": -1},
            {"b": None, "R": 1, "P": 0, "S": -3, "T": -1},
        ]
        multi_conf = dict(config_values['multilayer'], shared_nodes_ratio=shared_nodes_ratio, layers_config=layers_config)
        conf = dict(config_values, multilayer=multi_conf, update_str_type=update_rule, av_degree=k)
