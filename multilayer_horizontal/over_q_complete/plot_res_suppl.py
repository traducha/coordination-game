...
def plot_res(str_type=None, av_degree=None, res_dir=''):
    ...
    if str_type == const.REPLICATOR:
        plt.text(q_c_fit + 0.025, 0.15, f'$q_c=$ {round(q_c, 2)}', fontsize=9)
        plt.text(q_c_fit + 0.025, 0.05, r'$q_c^{fit}=$ ' + f'{round(q_c_fit, 2)}', fontsize=9)
    elif str_type == const.BEST_RESPONSE:
        plt.text(0.6, 0.15, f'$q_c=$ {round(q_c, 2)}', fontsize=9)
        plt.text(0.6, 0.05, r'$q_c^{fit}=$ ' + f'{round(q_c_fit, 2)}', fontsize=9)
    ...
    plt.figtext(0.14, 0.4, description, fontsize=9)
    plt.tight_layout()
