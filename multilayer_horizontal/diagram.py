...
def plot():
    ...
    x = np.linspace(-4, 0, 30)
    plt.plot(x, linear(x, -1, -3), color=const.VIOLET, alpha=1, linewidth=0.7)
    ...
    plt.fill_between([-4.5, 0], -4.5, [-3.5, 1], color=const.GREEN, alpha=0.7)
    plt.fill_between([-4.5, 0], 1, [-3.5, 1], color=const.BLUE, alpha=0.7)
    ...
    plt.text(-4.1, -2.08, "A is risk-dominant", fontsize=10, rotation=45, color='black')
    plt.text(-4.4, -1.75, "B is risk-dominant", fontsize=10, rotation=45, color='black')
    ...
    for T1 in np.linspace(-1, -3, 11)[1:]:
        ...
        S1 = round(-T1 - 3.0, 2)
        S2 = round(-T2 - 3.0, 2)
        plt.scatter(S1, T1, marker="o", color=const.VIOLET, s=18)
        plt.scatter(S2, T2, marker="o", color=const.VIOLET, s=18)
    ...
    for T1 in np.linspace(0, 0.45, 10):
        ...
        T2 = 1-T1
        S2 = -T2
        plt.scatter(S1, T1, marker="o", color=const.VIOLET, s=6)
        plt.scatter(S2, T2, marker="o", color=const.VIOLET, s=6)
    ...
    plt.text(-1.2, -1.68, r"$(S^{I}, T^{I})$", fontsize=10, color='black')
    plt.text(-2.8, -0.08, r"$(S^{II}, T^{II})$", fontsize=10, color='black')

    plt.text(-2.125, 1.15, r"$\Delta S$", fontsize=10, color='black')
    plt.text(0.15, -1.1, r"$\Delta T$", fontsize=10, color='black')

    plt.scatter(-2.8, -0.2, marker="o", color=const.YELLOW, s=50, facecolor='none')
    plt.scatter(-1.2, -1.8, marker="o", color=const.YELLOW, s=50, facecolor='none')
