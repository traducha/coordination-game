...
def nowakIM(x, d_t, config):
    k = 32
    S = config['S']
    T = config['T']

    c = (k+3.0 + 3.0*(S-T)) / ((k+3.0)*(k-2.0))

    d_x = (S+T-1.0) * x**3.0 + (1.0-2*S-T-c) * x**2.0 + (S+c) * x

    return d_x * d_t


def nowakBD(x, d_t, config):
    k = 3
    S = config['S']
    T = config['T']
...
def plot_repicator():
    ...
    res = []
    for T in np.linspace(-2, 1, 30):
        conf['T'] = T
        res_in = []
        for S in np.linspace(-3, 0, 30):
            conf['S'] = S
