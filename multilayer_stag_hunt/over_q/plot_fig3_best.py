...
def plot_res(str_type=None, av_degree=None, res_dir=''):
    ...
    if str_type == const.REPLICATOR:
        ...
        plt.text(q_c - 0.27, 0.11, f'$q_c=$ {round(q_c, 2)}', fontsize=9)
        plt.text(q_c - 0.27, 0.01, r'$q_c^{fit}=$ ' + f'{round(q_c_fit, 2)}', fontsize=9)
    ...
    plt.legend(loc='best', bbox_to_anchor=(0.75, 0.55, 0.25, 0.5), handles=legend_elements)
    plt.xlabel(r'$q$')
    ...
    plt.figtext(0.14, 0.45, description, fontsize=9)
