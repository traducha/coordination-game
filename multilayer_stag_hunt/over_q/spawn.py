...
if __name__ == '__main__':
    ...
    for T1 in [0.3]:  # np.linspace(0, 0.45, 10):
        T1 = round(T1, 2)
        S1 = -T1
        T2 = 1-T1
        S2 = -T2

        results_dir = f"res/long_res_{rules[update_str_type]}_k{k}_gap{round(S1-S2, 2)}"
        ...
        for i, node_overlap in enumerate(node_overlap_list):
            ...
            if os.path.isfile(f_name):
                ...
                command = f'run -t 100:00 -o {out_file} -e {er_file} {py_path} {script} {node_overlap} {update_str_type} {results_dir} {T1} {S1} {T2} {S2} {k}'
                os.system(command)
